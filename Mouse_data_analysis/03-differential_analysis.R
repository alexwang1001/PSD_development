...
data_imp
data_imp$condition <- factor(data_imp$condition, levels = c("P0", "P9", "P13", "P18", "P36"))

#using limma
condition <- factor(data_imp$condition, levels = c("P0", "P9", "P13", "P18", "P36"))
design <- model.matrix(terms(~ condition))
colnames(design)
colnames(design)[2:5] <- c("P9", "P13", "P18", "P36")
colnames(design)
...
table <- limma_res %>% select(rowname, logFC, CI.L, CI.R, t, 
                              P.Value, adj.P.Val, comparison) %>% mutate(comparison = paste0(comparison,
                                                                                             "_vs_P0")) %>% gather(variable, value, -c(rowname, 
...
pca_df$condition <- factor(pca_df$condition, levels = c("P0", "P9", "P13", "P18", "P36"))
plot2 <- ggplot(pca_df, aes(get(paste0("PC", 1)), get(paste0("PC", 2)))) +
  ...
  theme_DEP1() +
  geom_point(aes(col = fct_relevel(condition, "P0", "P9", "P13", "P18", "P36")),
             size = 2) +
  labs(col = "condition") +
...
#the heatmap of significant genes
