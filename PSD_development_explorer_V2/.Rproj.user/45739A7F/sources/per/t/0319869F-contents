...
#human PFC plot
plot_human_PFC <- function(dep, proteins) {
  ...
  if (length(proteins) > 1) {
    ...
    p <- ggplot(data = df, mapping = aes(x = Log2AgeDays, y = val), color = "black", fill = "black") +
      geom_smooth(color = "#F8766D", se = F, show.legend = F) +
...
#human V1 plot
plot_human_V1 <- function(dep, proteins) {
  row_data <- rowData(dep, use.names = FALSE)
  subset <- dep[proteins]
  df <- data.frame(assay(subset)) %>%
    rownames_to_column() %>%
  ...
  if (length(proteins) > 1) {
    ...
    p <- ggplot(data = df, mapping = aes(x = Log2AgeDays, y = val), color = "black", fill = "black") +
      geom_smooth(color = "#C77CFF", se = F, show.legend = F) +
      geom_point(size = 0.8, alpha = 0.8, show.legend = F) +
      theme(panel.grid.minor = element_blank(),
            panel.grid.major.y = element_blank(),
            panel.grid.major.x = element_line(colour = "gray80", linetype = "dashed"),
...
plot_macaque <- function(dep, proteins) {
  ...
  if (length(proteins) > 1) {
    ...
    p <- ggplot(data = df, mapping = aes(x = Log2AgeDays, y = val), color = "black", fill = "black") +
      geom_smooth(color = "#00BA38", se = F, show.legend = F, span = 1.1) +
      geom_point(size = 0.8, alpha = 0.8, show.legend = F) +
      theme(panel.grid.minor = element_blank(),
            panel.grid.major.y = element_blank(),
            panel.grid.major.x = element_line(colour = "gray80", linetype = "dashed"),
...
plot_mouse <- function(dep, proteins) {
  ...
  if (length(proteins) > 1) {
    ...
    p <- ggplot(data = df, mapping = aes(x = Log2AgeDays, y = val), color = "black", fill = "black") +
      geom_smooth(color = "#619CFF", se = F, show.legend = F, span = 1.1) +
      geom_point(size = 0.8, alpha = 0.8, show.legend = F) +
      theme(panel.grid.minor = element_blank(),
            panel.grid.major.y = element_blank(),
            panel.grid.major.x = element_line(colour = "gray80", linetype = "dashed"),
