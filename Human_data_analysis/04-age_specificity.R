...
dep_corrected_specificity <- dep_corrected
...
rowdata <- rowdata[,sort(c(grep("_diff", colnames(rowdata)), grep("_adj", colnames(rowdata)), c(1,2,3,4,113), grep("specificity_t", colnames(rowdata))))]

save(dep_corrected_specificity, file = "./Results/dep_corrected_specificity.RData")
