plot_single2_RNA <- function(se, genes, scale = FALSE, plot = TRUE, ylim = NULL) {
  ...
  if (scale) {
    ...
    df <- data.frame(assay(subset) - means) %>%
      rownames_to_column() %>%
      gather(geo_accession, val, -rowname) %>%
      left_join(., data.frame(colData(subset)), by = "geo_accession")
    colnames(df)[1] <- "Gene"
    if (length(genes) > 1) {
      p <- ggplot(data = df, mapping = aes(x = log2_days, y = val, color = Gene, fill = Gene)) +
      ...
      p <- ggplot(data = df, mapping = aes(x = log2_days, y = val), color = "black", fill = "black") +
    ...
    if(plot) {
      ...
      df <- df %>%
        select(Gene, geo_accession, age, log2_days, val)
      colnames(df) <- c("Gene", "geo_accession", "age",
                        "log2_days", "level")
    }
    return(df)
  } else {
