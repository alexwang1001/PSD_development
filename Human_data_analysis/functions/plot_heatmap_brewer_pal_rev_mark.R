library(DEP)
plot_heatmap_brewer_rev_mark <- function(dep, type = c("contrast", "centered"),
                                 kmeans = FALSE, k = 6,
  ...
  if(!is.null(indicate) & type == "centered") {
    ha1 <- get_annotation_brewer_pal_rev(dep, indicate)
  } else {
    ha1 <- NULL
  }
  ...
  genes_to_highlight <- c( "NEFL", "NEFH","NEFM","DLG1", "DLG2", "DLG3", "DLG4","DLG5", "GRIA2", "GRIN2A", "SHANK3", "LZTS1", "CDH2")
  genes_to_highlight <- genes_to_highlight[genes_to_highlight %in% rownames(df)]
  row_number_to_highlight <- vector(mode = "numeric")
  for (gene in genes_to_highlight) {
    name <- which(rownames(df) == gene)
    row_number_to_highlight <- c(row_number_to_highlight,name)
  }
  ha2 = rowAnnotation(foo = anno_mark(at = row_number_to_highlight, labels = genes_to_highlight, padding = 0.5))
