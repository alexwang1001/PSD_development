...
species_comparison_V1 <- readRDS("Results/species_comparison_humanized_age_based_on_V1.rds")

rowdata <- as.data.frame(rowData(species_comparison_V1))

brown <- species_comparison_V1[rowdata$Human_Module == "brown", ]
...
blue <- species_comparison_V1[rowdata$Human_Module == "blue", ]
...
yellow <- species_comparison_V1[rowdata$Human_Module == "yellow", ]
...
turquoise <- species_comparison_V1[rowdata$Human_Module == "turquoise", ]
...
#plot module mean levels in different species
...
ggplot(data = df_all_long, mapping = aes(x = humanized_log2_age_days, y = mean)) +
  geom_smooth(mapping = aes(color = species) ,se = F, span = 0.75, show.legend = T) +
...
ggsave("Results/species_comparison/module_abundance_based_on_V1_across_species.pdf", width = 6, height = 6)
ggsave("Results/species_comparison/module_abundance_based_on_V1_across_species.png", width = 6, height = 6)
